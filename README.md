...
`cmatrix -c` prints the movie's half-width katakana (U+FF66–U+FF9D), mixed with numerals, Latin letters (upper and lower case), and symbols as seen in the film (roughly 30% of characters). The leading (head) character follows the color set with `-C` rather than always printing white. For it to display correctly you need all three of:
